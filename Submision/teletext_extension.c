...
void set_board( Cell board[HEIGHT][WIDTH]){
   ...
   for (i=0; i<HEIGHT; i++){
      for (j=0; j<WIDTH; j++){
         ...
         /* extension */
         board[i][j] = init_infinity(board[i][j]);
         if (j == 0){
...
/* extension */
Cell init_infinity( Cell c){

   c.column.population = 1;
   c.column.position = 0;
   c.row.population = 1;
   c.row.position = 0;
   return c;
...
Cell exam_cases(Cell c, Cell *prev){
   ...
   if (( c.character >= RED) && (c.character <= LGREEN)){
      prev->Mode = c.Mode = Alphanumeric;
      prev->Held_graph = c.Held_graph = No;
   ...
   /* extension */
   if (c.character == INFINITESIZE){
      prev->Height = c.Height = Infinite;
      prev->Held_graph = c.Held_graph = No;
   }
   return c;
}
...
/* extension */
void set_infinity( Cell board[HEIGHT][WIDTH]){

   int i, j;

   for (i=0; i<HEIGHT; i++){
      for (j=0; j<WIDTH; j++){
         if (board[i][j].Height == Infinite){
            board[i][j] = check_neighboors(i, j, board);
         }
      }
   }
}
...
/* extension */
Cell check_neighboors(int y, int x, Cell board[HEIGHT][WIDTH]){

   int count=1;
   /*check left*/
   while((board[y][x-count].character == board[y][x].character)
   && (board[y][x-count].Height == Infinite)){
      board[y][x].column.population++;
      board[y][x].column.position++;
      count++;
   }
   /*right*/
   count = 1;
   ...
   /*up*/
   count = 1;
   while((board[y-count][x].character == board[y][x].character)
   && (board[y-count][x].Height == Infinite)){
      board[y][x].row.population++;
      board[y][x].row.position++;
      count++;
   }
   /*down*/
   count = 1;
...
void draw_cell(Window *sw, fntrow fontdata[FNTCHARS][FNTHEIGHT], Cell c, int x, int y, int height_of_above){
   ...
   int i=0, k=0, j, height_to_draw = FNTHEIGHT, width_to_draw = FNTWIDTH;
   ...
   /* extension */
   if (c.Height == Infinite){
      /*height settings*/
      height_to_draw /= c.row.population;
      i = height_to_draw * c.row.position;
      height_to_draw += i;
      /*width settings*/
      width_to_draw /= c.column.population;
      k = width_to_draw * c.column.position;
      width_to_draw += k;
      y -= FNTHEIGHT * c.row.position; /*shift height appropriately*/
      x -= FNTWIDTH * c.column.position ; /*shift width appropriately*/
   }
   for( ; i < height_to_draw; i++){
      for( j=k ; j < width_to_draw; j++){
...
void draw_foreground(Window *sw, Cell c, int x, int j, int y, int i){
   ...
   int error=0, h, w, rows, colms;

   colms = c.column.population;
   rows = c.row.population;
   select_colour(sw, c.foreColour);
   if (c.Height == Single){
      error += SDL_RenderDrawPoint(sw->renderer, x+j , y + i );
   }
   else if (c.Height == Double){
      error += SDL_RenderDrawPoint(sw->renderer, x+j , y + i*2);
      error += SDL_RenderDrawPoint(sw->renderer, x+j , y +  i*2+1);
   }/* extension */
   else if(c.Height == Infinite){
      for (h=0; h < rows; h++){
         for (w=0; w < colms; w++){
            error += SDL_RenderDrawPoint(sw->renderer, x+ j*colms+w , y+ i*rows+h);
         }
      }
   }
   if(error != 0){
...
void select_colour( Window *sw, int colour ){
   ...
   switch (colour) {
      ...
      case BLACK:
         ...
         break;
      case TEAL:
         error += SDL_SetRenderDrawColor(sw->renderer, 0, 176, 139, SDL_ALPHA_OPAQUE);
         break;
      case ORANGE:
         error += SDL_SetRenderDrawColor(sw->renderer, 255, 94, 0, SDL_ALPHA_OPAQUE);
         break;
      case PINK:
         error += SDL_SetRenderDrawColor(sw->renderer, 214, 37, 152, SDL_ALPHA_OPAQUE);
         break;
      case LGREEN:
         error += SDL_SetRenderDrawColor(sw->renderer, 151, 215, 0, SDL_ALPHA_OPAQUE);
         break;
   }
   if(error !=0){
